...
class Period:
    ...
    def overlapping_days(self, budget):
        ...
        overlapping_end = last_day if last_day < self.end else self.end
        overlapping_start = first_day if first_day > self.start else self.start
    ...
    def query(self, start: date, end: date) -> float:
        ...
        if start.strftime('%Y%m') == end.strftime('%Y%m'):
            ...
            while current < end.replace(day=1) + relativedelta(months=1):
                ...
                total_amount += period.overlapping_days(budget) * budget.daily_amount()
